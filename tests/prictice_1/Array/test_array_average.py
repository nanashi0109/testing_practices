...
@pytest.mark.parametrize("value_array, expend_result",
                         [
                             ([2, 5, 3, 6], 4),
                             ([2.5, 6.3, 1.2, 9.4], 4.85),
                             ([-5, -2, -6, -2, -8], -4.6),
                             ([-5, 6, -1, 3, -2, 11], 2),
                             ([5, -5.3, 7, 9.1, -6.9], 1.78),
                         ])
def test_average_positive(value_array, expend_result):
...
def test_average_negative(value_array, expend_result):
    arr = Array(value_array)
...
def test_average_boundary(value_array, expend_result):
    arr = Array(value_array)
